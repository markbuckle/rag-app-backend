...
def submit_query_endpoint(request: SubmitQueryRequest) -> QueryResponse:
    ...
    new_query = QueryModel(
        ...
        response_text="",  # Initialize with default empty string
        sources=[],  # Initialize with default empty list
    )  # could also add , user_id=user_id
    ...
    # Create a QueryResponse to return
    new_query_response = QueryResponse(
        query_text=request.query_text,
        response_text=new_query.answer_text,
...
def invoke_worker(query: QueryResponse):
